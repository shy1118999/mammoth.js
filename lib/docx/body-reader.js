...
function BodyReader(options) {
    ...
    function readParagraphProperties(element) {
        var line = element.firstOrEmpty("w:spacing").attributes["w:line"] || null;
        if (line) {
            var lineRule = element.firstOrEmpty("w:spacing").attributes["w:lineRule"] || null;
            if (lineRule === 'auto') {
                line = "" + (line / 240).toFixed(2);
            } else if (lineRule === 'atLeast') {
                line = "calc(100% + "+line / 20+"pt)";
            } else {
                line = ""+line / 20+"pt";
            }
        }
    ...
    function readRunProperties(element) {
        return readRunStyle(element).map(function (style) {
            ...
            return {
                ...
                isEmphasis: element.firstOrEmpty("w:em").attributes["w:val"] === "dot",
            };
        });
    }
    ...
    function readParagraphStyle(element) {
        const res = readStyle(element, "w:pStyle", "Paragraph", styles.findParagraphStyleById);
        return res;
    }
